...
app = FastAPI(title="Zapstrix Voice Agent", version="3.0.1")
...
async def media_stream(websocket: WebSocket):
    ...
    try:
        ...
        async with websockets.connect(
            # Latest Realtime endpoint (no beta header)
            f"wss://api.openai.com/v1/realtime?"
            f"model=gpt-4o-realtime-preview&temperature={TEMPERATURE}",
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Zapstrix Voice Agent on port {PORT}")
